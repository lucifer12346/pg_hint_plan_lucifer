...
make_join_rel(PlannerInfo *root, RelOptInfo *rel1, RelOptInfo *rel2)
{
	...
	// ereport(NOTICE,(errmsg("make_join_rel rels: %ld   %ld  %ld", rel1->relids->words[0], rel2->relids->words[0], joinrelids->words[0])));
	...
	// ereport(NOTICE,(errmsg("making join %d %d", rel1->relid, rel2->relid)));
	/*
	...
	if (reversed)
	{
		...
		// ereport(pg_hint_plan_parse_message_level,(errmsg("REVERSED"))); 
		rel1 = rel2;
		rel2 = trel;
	}
	...
	if (sjinfo == NULL)
	{
		// ereport(pg_hint_plan_parse_message_level,(errmsg("sjinfo null"))); 
		sjinfo = &sjinfo_data;
	 ...
	 */
	// ereport(NOTICE,(errmsg("build join rel join %d %d", rel1->relid, rel2->relid)));
	...
	/* !!! this part aims to implement the rows() hints !!! */
	{
		...
		if (justforme)
		{
			// ereport(NOTICE,(errmsg("justforme row hints")));
			/*
		...
		else
		{
			if (domultiply)
			{
				 ...
				 */
				ereport(NOTICE,(errmsg("domultiply row hints")));
							...
							SpecialJoinInfo *sjinfo, List *restrictlist)
{
	 ...
	 * JOIN_INNER.
	 */
	// ereport(NOTICE,(errmsg("populate_joinrel_with_paths %d %d", rel1->relid, rel2->relid)));
	switch (sjinfo->jointype)
	{
		case JOIN_INNER:
			...
			break;
		case JOIN_LEFT:
			// ereport(NOTICE,(errmsg("JOIN_LEFT")));
			...
			break;
		case JOIN_FULL:
		// ereport(NOTICE,(errmsg("JOIN_FULL")));
			if ((is_dummy_rel(rel1) && is_dummy_rel(rel2)) ||
				restriction_is_constant_false(restrictlist, joinrel, true))
			{
				mark_dummy_rel(joinrel);
			...
			break;
		case JOIN_SEMI:
		// ereport(NOTICE,(errmsg("JOIN_SE/MI")));
			...
			break;
		case JOIN_ANTI:
		// ereport(NOTICE,(errmsg("JOIN_ANTI")));
			if (is_dummy_rel(rel1) ||
				restriction_is_constant_false(restrictlist, joinrel, true))
			{
				mark_dummy_rel(joinrel);
