 ...
 */
static void
set_plain_rel_pathlist(PlannerInfo *root, RelOptInfo *rel, RangeTblEntry *rte, ScanMethodHint   *shint)
{
	...
	shint->base.state = HINT_STATE_USED;
	if(shint->enforce_mask != ENABLE_SEQSCAN && shint->enforce_mask != ENABLE_TIDSCAN){
...
standard_join_search(PlannerInfo *root, int levels_needed, List *initial_rels)
{
	...
	for (lev = 2; lev <= levels_needed; lev++)
	{
		...
		if (root->join_rel_level[lev] == NIL && complete_hints == 0)
			elog(ERROR, "No %d-way joins", lev);
	}
...
join_search_one_level(PlannerInfo *root, int level)
{
	...
	ListCell   *r;
	int			k;
	int  		clause_num, clauseless_num, bushy_num;

	if (DEBUG)
		ereport(NOTICE,(errmsg("-------------start join search level %d---------------", level))); 
	...
	if (DEBUG)
		ereport(NOTICE,(errmsg("first step num %d %d", clause_num, clauseless_num)));
	/*
	...
	for (k = 2;; k++)
	{
		...
		foreach(r, joinrels[k])
		{
			 ...
			 */
			if (old_rel->joininfo == NIL && !old_rel->has_eclass_joins &&
				!has_join_restriction(root, old_rel) && complete_hints == 0){
				continue;
			}
			...
			for_each_cell(r2, other_rels_list, other_rels)
			{
				...
				if (!bms_overlap(old_rel->relids, new_rel->relids))
				{
					...
					if (have_relevant_joinclause(root, old_rel, new_rel) ||
						have_join_order_restriction(root, old_rel, new_rel) || complete_hints == 1)
					{
	...
	if (DEBUG)
		ereport(NOTICE,(errmsg("bushy num: %d", bushy_num))); 
	/*----------
	 ...
	 *----------
	 */
	if (joinrels[level] == NIL && complete_hints == 1){
		ereport(pg_hint_plan_parse_message_level,(errmsg("failed to find any usable joins When Completion hints"))); 
	}
	if (joinrels[level] == NIL && complete_hints == 0)
	{
		ereport(pg_hint_plan_parse_message_level,(errmsg("failed to find any usable joins"))); 
		/*
		...
		if (joinrels[level] == NIL &&
			...
			!root->hasLateralRTEs)
			elog(ERROR, "failed to build any %d-way joins in one level", level);
	}
}
						  ...
						  ListCell *other_rels)
{
	ListCell   *l;
	Relids			joinrelids;
	JoinMethodHint *rel1_join_hint, *rel2_join_hint;
	...
	for_each_cell(l, other_rels_list, other_rels)
	{
		...
		if (!bms_overlap(old_rel->relids, other_rel->relids) &&
			(have_relevant_joinclause(root, old_rel, other_rel) ||
			 have_join_order_restriction(root, old_rel, other_rel)|| complete_hints == 1))
		{
