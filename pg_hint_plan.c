...
/*seq scan keyword of enum type*/

typedef enum HintScan
{
	SEQSCAN = 0x01,
	INDEXSCAN = 0x02,
	INDEXONLYSCAN = 0x03
...
//  For our hints comletion check --lucifer
// We use complete hints to record whether a hint contains all scan hints, join hints and leading hints with a correcct syntax.
int complete_hints = 0;
int DEBUG = 1;
char* JoinHints[] = {
	"NestLoop",
	"HashJoin", 
...
int extractContents(char* str, char** JoinSet, int joinSize, char ** ScanSet, int scanSize);
int CompleteHints(const char* hints);
void strip(char *str);
void add_parentheses(char* str);

//  Our comletion check ends here--lucifer
...
static void set_plain_rel_pathlist(PlannerInfo *root, RelOptInfo *rel,
								   RangeTblEntry *rte, ScanMethodHint   *shint);
...
parse_hints(HintState *hstate, Query *parse, const char *str)
{
	...
	complete_hints = CompleteHints(str);
	if(DEBUG)
		ereport(pg_hint_plan_parse_message_level,(errmsg("hint: \n%s , complete: %d",str, complete_hints)));
	initStringInfo(&buf);
	while (*str != '\0')
	{
						...
						GucContext context)
{
	...
	if (enforce_mask == ENABLE_HASHJOIN)
	{
		...
		if (new_multipler >= 1000)
			new_multipler = 2; // a modification by lucifer to ensure this does not affect index scan
				 ...
				 bool using_parent_hint)
{
	...
	if (hint->enforce_mask == ENABLE_SEQSCAN ||
		hint->enforce_mask == ENABLE_TIDSCAN)
	{	
		/* a modification on implementation of sequential scan by lucifer
		*/
 ...
 */
static JoinMethodHint *
find_join_hint_with_rel(Relids joinrelids, Relids relids)
{
	List	   *join_hint;
	ListCell   *l;
	...
	foreach(l, join_hint)
	{
		...
		if (bms_is_subset( relids, hint->joinrelids)){
			return hint;
		}
	}
	...
	PlannerInfo *root, List *initial_rels, HintState *hstate, int nbaserel)
{
	...
	if (DEBUG)
		ereport(NOTICE,(errmsg("\n outer_rels %s\n inner_rels %s", outer_rels->relation,  inner_rels->relation)));
	...
	inner_relids = OuterInnerJoinCreate(inner_rels,
										...
										nbaserel);
	if (DEBUG)
		ereport(NOTICE,(errmsg("\n length %d %d", bms_num_members(outer_relids), bms_num_members(inner_relids))));
		...
		List *initial_rels, JoinMethodHint **join_method_hints)
{
	...
	for (i = 0; i < hstate->num_hints[HINT_TYPE_LEADING]; i++)
	{
		...
		foreach(l, leading_hint->relations)
		{
			...
			relid = find_relid_aliasname(root, relname, initial_rels,
										 leading_hint->base.hint_str);
			if(DEBUG)
				ereport(NOTICE,(errmsg("relid in finding : %d, %s", relid, relname)));
			if (relid == -1)
	...
	if (lhint == NULL){
		ereport(NOTICE,(errmsg("lhint null")));
		complete_hints = 0;
		return false;
	}
	...
	{
		ereport(NOTICE,(errmsg("lhint->outer_inner null")));
	...
	{
		ereport(NOTICE,(errmsg("create Outer InnerJoin Create")));
		...
		Assert(njoinrels >= 2);
		if (njoinrels != nbaserel){
			ereport(NOTICE,(errmsg("not same %d, %d", njoinrels, nbaserel)));
			complete_hints = 0;
		}
		ereport(NOTICE,(errmsg("njoinrels %d  nbaserel %d", njoinrels, nbaserel )));
		/*
		 * Delete all join hints which have different combination from Leading
		 * hint.
		 */
...
make_join_rel_wrapper(PlannerInfo *root, RelOptInfo *rel1, RelOptInfo *rel2)
{
	...
	/* return when hints is completed but no hint found */
	if (complete_hints == 1 && !join_hint){
		ereport(NOTICE,(errmsg("end? %d", joinrelids->words[0])));
		return NULL;
	}
							 ...
							 List *restrictlist)
{
	...
	JoinMethodHint *join_hint;
	JoinMethodHint *memoize_hint;
	JoinMethodHint *inner_join_hint, *outer_join_hint;
	int level;
	int				save_nestlevel;
	...
	level = bms_num_members(joinrelids);
	...
	if (join_hint || memoize_hint)
	{
		...
		if (join_hint)
		{
			temp = bms_intersect(innerrel->relids, root->all_baserels);
			// temp
						 ...
						 List *initial_rels)
{
	...
	if (!current_hint_state || hint_inhibit_level > 0)
	{
		if (prev_join_search){
			...
			if (DEBUG)
				ereport(NOTICE,(errmsg("use geqo")));
			return geqo(root, levels_needed, initial_rels);}
		else{
			if (DEBUG)
				ereport(NOTICE,(errmsg("standard_join_search")));
							  ...
							  Index rti, RangeTblEntry *rte)
{
	...
	if (found_hints & HINT_BM_SCAN_METHOD || found_hints & HINT_BM_PARALLEL)
	{
		...
		else
		{
			...
			shint = find_scan_hint(root, rel->relid);
			set_plain_rel_pathlist(root, rel, rte, shint);
...
//for our completion check --lucifer
...
void addString(char*** array, int* size, const char* newString) {
    ...
    if (temp == NULL) {
        ereport(NOTICE,(errmsg("freeing Failed to reallocate memory ")));
        return; 
    }
    *array = temp;
	...
	if ((*array)[*size] == NULL) {
		ereport(NOTICE,(errmsg("Failed to allocate memory for new string")));
		return;
	}
    (*size)++;
}
...
void replaceSpaces(char* str) {
    ...
    int inSpace = 0;
	char* temp;
	temp = malloc((strlen(str)+10)* sizeof(char));
...
void strip(char *str) {
    if (!str) return;
    const char *remove_set = " \t\n\r\v\f";
    size_t start = 0;
    size_t end = strlen(str);
    while (start < end && strchr(remove_set, str[start])) 
        start++;
    
    while (end > start && strchr(remove_set, str[end - 1])) 
        end--;
    size_t new_length = end - start;
    char *result = (char *)malloc((new_length + 10)*sizeof(char));
    if (!result) return;
    strncpy(result, str + start, new_length);
    result[new_length] = '\0';
	strcpy(str, result);
	str[new_length] = '\0';
	free(result);
...
int CompleteHints(const char* hints) {
	...
	char* inner; 
	size_t innerLength;
	prefix = malloc((strlen(hints)+10)*sizeof(char));
	inner = malloc((strlen(hints)+10)*sizeof(char));
	Joins = NULL;
	joinSize = 0;
	Scans = NULL;
	scanSize = 0;
	haveLeadings = 0;
	tempStr = strdup(hints);
    line = strtok(tempStr, "\n"); 
    while (line != NULL) {
		strip(line);
		ereport(NOTICE,(errmsg("line %s", line)));
        openParen = strchr(line, '(');
		if (openParen) {
			prefixLength = openParen - line; 
			strncpy(prefix, line, prefixLength);
			prefix[prefixLength] = '\0';
			closeParen = strrchr(openParen, ')');
			if (closeParen) {
				innerLength = closeParen - openParen - 1; 
				strncpy(inner, openParen + 1, innerLength);
				inner[innerLength] = '\0'; 
				strip(inner);
				removeSpaces(prefix);
				if (isStringInArray(prefix, JoinHints, 3)){
					addString(&Joins, &joinSize, inner);
				}
				else if (isStringInArray(prefix, ScanHints, 5)){
					removeSpaces(inner);
					addString(&Scans, &scanSize, inner);
				}
				if (strcmp(prefix, "Leading") == 0){
					leadings = malloc((innerLength + 10)*sizeof(char));
					ereport(NOTICE,(errmsg("inner : %s", inner)));
					strcpy(leadings, inner);
					leadings[innerLength] = '\0';
					haveLeadings = 1;
				}
				
			} else {
				ereport(NOTICE,(errmsg("Prefix: %s, No closing parenthesis found!\n", prefix)));
			}
		} else {
			ereport(NOTICE,(errmsg("No parentheses found in line: %s\n", line)));
		}
        line = strtok(NULL, "\n"); 
    }
	if (haveLeadings == 1){
		ereport(NOTICE,(errmsg("Leadings not null")));
	}
	if ((haveLeadings == 0) || (extractContents(leadings, Joins, joinSize, Scans, scanSize) == 0)){
		free(prefix);
		free(inner);
		if (haveLeadings == 1)
			free(leadings);
		freeStringArray(Scans, &scanSize);
		freeStringArray(Joins, &joinSize);
		return 0;
	}
    free(prefix);
	free(inner);
...
void add_parentheses(char* str) {
    ...
    size_t new_len = original_len + 2 + 1; 
    char* result = (char*)malloc((new_len + 10 )* sizeof(char));
    if (result == NULL) {
        perror("Memory allocation failed");
    }
    result[0] = '('; 
    strcpy(result + 1, str); 
    result[original_len + 1] = ')';
    result[new_len - 1] = '\0';
	strcpy(str, result);
	str[new_len - 1] = '\0';
    free(result);
...
int extractContents(char* str, char** JoinSet, int joinSize, char ** ScanSet, int scanSize) {
	...
	char* temp;
	int havingTemp;
	ereport(NOTICE,(errmsg("extractContents\n")));
    initStack(&stack);
	strip(str);
	if(str[0] != '('){
		ereport(NOTICE,(errmsg("extractContents with no first parentheses \n")));
		add_parentheses(str);
	}
    ptr = str;
    havingTemp = 0;
    ...
    while (*ptr) {
        if (*ptr == '(') {
            ...
            if (!isEmpty(&stack)) {
                char* start = pop(&stack);
                size_t length = ptr - start - 1;
                if (length > 0) {
					temp = malloc((length+10)*sizeof(char));
    ...
    while (token != NULL) {
		if (! isStringInArray(token, ScanSet, scanSize)){
			free(temp);
			return 0;
		}
        token = strtok(NULL, " ");
...
//our completion check ends here --lucifer
